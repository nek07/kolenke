...
from kolenke.db.connection import query
from kolenke.db.repositories import settings
from kolenke.services import mailer, pipeline


def stage(vid):
    return query("SELECT stage FROM vacancies WHERE id=?", (vid,))[0]["stage"]
...
def test_stage_events_are_recorded():
    ...
    assert query("SELECT kind, text FROM events WHERE vacancy_id=?", (v,)) == [{"kind": "stage", "text": "Этап: Приглашение"}]


def test_invalid_next_date_is_rejected_and_missing_vacancy():
    v = add_vacancy(status="applied")
    with pytest.raises(ValueError):
        pipeline.update(v, {"next_step": "Собеседование", "next_at": "<img src=x>"})
    with pytest.raises(pipeline.NotFound):
        pipeline.update(99999, {"stage": "offer"})
...
def test_followup_after_n_days_without_reaction():
    ...
    add_vacancy(ext_id="2", status="applied", applied_at=datetime.now().isoformat(timespec="seconds"))
    add_vacancy(ext_id="3", status="applied", applied_at=old, hh_state="приглашение")
    add_vacancy(ext_id="4", status="applied", applied_at=old, followup="sent")
    pipeline.ensure_stages()
...
def test_email_build_with_attachment(tmp_path):
    ...
    settings.save({"full_name": "Аня", "smtp_user": "anya@gmail.com", "resume_file": str(cv), "desired_position": "Аналитик"})
    msg = mailer.build(settings.get(), {"name": "Halyk Bank", "email": "hr@halyk.kz", "position": ""})
